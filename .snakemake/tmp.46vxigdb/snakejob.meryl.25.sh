#!/bin/bash
# properties = {"type": "single", "rule": "meryl", "local": false, "input": ["/mnt/cbib/pangenoak_trials/GenomAsm4pg/lady_bug_data.fasta.gz"], "output": ["/mnt/cbib/pangenoak_trials/GenomAsm4pg/tutorial_output/results/toy_dataset_bug/toy_test_run/02_genome_assembly/01_raw_assembly/01_assembly_QC/merqury/toy_dataset_bug_reads-db_k21.meryl"], "wildcards": {"runid": "toy_dataset_bug/toy_test_run", "id": "toy_dataset_bug"}, "params": {}, "log": [], "threads": 20, "resources": {"tmpdir": "/tmp", "mem_mb": 60000}, "jobid": 25, "cluster": {"job-name": "meryl", "time": "96:00:00", "ntasks": 1, "cpus-per-task": 10, "mem": "60G", "nodes": 1, "ntasks-per-node": 1, "output": "slurm_logs/meryl.%N.%j.out", "error": "slurm_logs/meryl.%N.%j.err", "mail-type": "END,FAIL", "mail-user": "lucien.piat@inrae.fr"}}
 cd /isilon/cbib/pangenoak_trials/GenomAsm4pg && \
/module/apps/snakemake/5.8.1/bin/python \
-m snakemake /mnt/cbib/pangenoak_trials/GenomAsm4pg/tutorial_output/results/toy_dataset_bug/toy_test_run/02_genome_assembly/01_raw_assembly/01_assembly_QC/merqury/toy_dataset_bug_reads-db_k21.meryl --snakefile /isilon/cbib/pangenoak_trials/GenomAsm4pg/workflow/Snakefile \
...
--wrapper-prefix https://github.com/snakemake/snakemake-wrappers/raw/ \
   --allowed-rules meryl --nocolor --notemp --no-hooks --nolock --scheduler-solver-path /module/apps/snakemake/5.8.1/bin \
